...
# --- Compute derived features (must match 3-train.py) ---
DERIVED_FEATURES = {
...
for name, fn in DERIVED_FEATURES.items():
    if all(c in df.columns for c in ["nb_getters", "nb_setters", "nb_methods", "class_loc",
                                      "cyclomatic_complexity", "nb_attributes", "comment_loc",
                                      "nb_method_calls", "nb_external_dependencies"]):
        df[name] = fn(df).clip(0, 100)
if not JSON_OUTPUT:
    print(f"[INFO] Added {len(DERIVED_FEATURES)} derived features")
