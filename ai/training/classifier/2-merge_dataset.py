...
This script merges labeled classifications with the original dataset.
It handles label normalization, filtering of invalid rows, and conversion
of labels to numeric IDs based on the label definition file.

Usage:
    python 2-merge_dataset.py <input_csv> <labeled_csv> <output_csv> [--labels-def=<path>]

Example:
    python 2-merge_dataset.py dataset/samples.csv classified_output/classified_roles.csv dataset/final_dataset.csv
"""
import pandas as pd
import sys
import os
...
parser.add_argument("--labels-def", help="Labels definition CSV (roles.csv)", 
                    default="labels/roles.csv")
