#!/usr/bin/env python3
"""
Remote LLM Labeling Script (OpenAI API)

This script uses OpenAI's API to classify code snippets in batches.
More efficient than local LLM for large datasets, but requires an API key.

Usage:
    export OPENAI_API_KEY=your_api_key
    python 1-labelize_remote.py <input_csv> [--count=<n>] [--output-dir=<dir>] [--labels-dir=<dir>]
...
Example:
    python 1-labelize_remote.py dataset/samples.csv --count=10000 --output-dir=classified_output

Note:
    Processes samples in batches of 100 for efficiency.
"""
import csv
import os
import sys
...
if not api_key:
    ...
    print("[INFO] Please set it with: export OPENAI_API_KEY=your_api_key")
    sys.exit(1)
...
def load_labels(path):
    ...
    Args:
        path: Path to the CSV file containing labels
        
    Returns:
        List of label strings, optionally with descriptions
    """
    labels = []
...
def read_snippet(filepath, max_lines=60):
    ...
    Returns:
        String containing the file snippet, or empty string if file not found
    """
...
def detect_language(filepath):
    ...
    Returns:
        Language identifier string (e.g., 'php', 'python', 'go')
    """
    if not filepath:
        return ""
...
def build_system_prompt(labels: List[str]) -> str:
    """
    Build system prompt for the LLM with label list.
...
def build_user_prompt(batch: List[tuple]) -> str:
    ...
    Args:
        batch: List of tuples (class_name, file_path)
        
    Returns:
        User prompt string with code snippets
    """
    items = []
    file_index = 0
...
def classify_batch(batch: List[tuple], labels: List[str], system_prompt: str) -> List[Dict]:
    ...
    Args:
        batch: List of tuples (class_name, file_path) to classify
        labels: List of available classification labels
        system_prompt: System prompt for the LLM
        
    Returns:
        List of dictionaries with 'class', 'file', and 'label' keys.
        Returns UNKNOWN for all items if classification fails.
    """
