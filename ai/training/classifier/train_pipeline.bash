...
# HuggingFace dataset
HF_BASE_URL="https://huggingface.co/datasets/Halleck45/code-classification/resolve/main"
...
# 3. Get labels (from HuggingFace or local)
echo ""
LABELED_CSV="$CLASSIFIED_DIR/classified_roles.csv"
if [ -n "$LOCAL_LABELS" ]; then
    echo "[3/5] Using local labels: $LOCAL_LABELS"
    cp "$LOCAL_LABELS" "$LABELED_CSV"
else
    echo "[3/5] Downloading labels from HuggingFace..."
    curl -fSL "$HF_BASE_URL/$LANGUAGE/classified_roles.csv" -o "$LABELED_CSV"
    ROWS=$(wc -l < "$LABELED_CSV")
    echo "Downloaded $((ROWS - 1)) labels"
fi
