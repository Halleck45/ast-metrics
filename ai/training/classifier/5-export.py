...
Output:
    - model.json.gz: Compressed JSON model file for Go runtime
"""
import sys
import os
import json
...
def export_model(model_path, encoders_path, features_path, output_path):
    """
    Export a trained model to JSON format for Go runtime.
    
    Args:
        model_path: Path to the trained model (.pkl file)
        encoders_path: Path to the encoders file (.joblib file)
        features_path: Path to the features metadata file (.json file)
        output_path: Path where the exported JSON model will be saved
    """
