...
func (c *AIDatasetCommand) Execute() error {
	...
	// Note: dump options are now handled per-runner via configuration
	...
	// Parse and analyze all AST files
	parsedFiles, parseErr := engine.ParseFiles(config, c.runners)
	if parseErr != nil {
		return fmt.Errorf("failed to parse files: %w", parseErr)
	}
