...
// NOTE: token_pattern used in Python is \b\w{2,}\b (letters+digits+underscore, len>=2)
// Here we approximate with [a-z0-9_]+, len>=2 (after lowercase if enabled).
func tokenizeBasic(text string, lowercase bool, stopWords string) []string {
	if lowercase {
		text = strings.ToLower(text)
	}

	// tiny english stopwords set (good enough; avoids big payload)
	// If you want exact sklearn list, export it explicitly and embed it.
...
// generateNgrams produces all n-grams from minN to maxN (inclusive) from a list of unigram tokens.
// N-grams are formed by joining consecutive tokens with a space, matching sklearn's default behavior.
// For example, tokens=["foo","bar","baz"] with minN=1, maxN=3 produces:
...
func generateNgrams(tokens []string, minN, maxN int) []string {
	...
	n := len(tokens)
	// Estimate capacity: sum of (n - k + 1) for k in [minN, maxN]
	cap := 0
	for k := minN; k <= maxN && k <= n; k++ {
		cap += n - k + 1
	}
...
func englishStopwords() map[string]struct{} {
	// Must match sklearn.feature_extraction.text.ENGLISH_STOP_WORDS (318 words)
	// to produce identical TF-IDF features as the trained model.
	words := []string{
