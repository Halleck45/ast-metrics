...
type FeaturesMetadata struct {
	...
	NumericalColsOrder        []string            `json:"numerical_cols_order"`
	HashingNFeatures          int                 `json:"hashing_n_features"`
	ClassNameWeight           float64             `json:"class_name_weight"`
	ProgrammingLanguageWeight float64             `json:"programming_language_weight"`
	DerivedFeatures           []string            `json:"derived_features,omitempty"`
	NlpWeights                map[string]float64  `json:"nlp_weights,omitempty"`
}

type TfidfVectorizer struct {
	Type        string         `json:"type"` // "hashing_tfidf" or "tfidf_vocab"
	...
	// hashing-specific
	AlternateSign bool   `json:"alternate_sign,omitempty"`
	Lowercase     bool   `json:"lowercase,omitempty"`
	TokenPattern  string `json:"token_pattern,omitempty"`
	NgramRange    []int  `json:"ngram_range,omitempty"` // e.g. [1,3] for unigrams+bigrams+trigrams
	StopWords     any    `json:"stop_words,omitempty"`  // can be "english" or null
}
...
// First tries to load from embedded models, then falls back to filesystem
func LoadModel(path string) (*RandomForestModel, error) {
	// Try to load from embedded models first
	// Extract language from path (e.g., "ai/training/classifier/build/php/model.json" -> "php")
	// The path is typically: {ModelDir}/{lang}/model.json
	// where ModelDir might be "ai/training/classifier/build"
	var lang string
	// Remove .gz extension if present
	cleanPath := strings.TrimSuffix(path, ".gz")
	// Get directory containing model.json
	dir := filepath.Dir(cleanPath)
	// Extract the last directory component (should be the language)
	lang = filepath.Base(dir)
	// Normalize to lowercase
	lang = strings.ToLower(lang)

	// Try embedded model first (skip if lang is empty or looks like a path component)
	if lang != "" && lang != "." && lang != "build" && lang != "classifier" && lang != "training" {
		embeddedPath := "models/" + lang + "/model.json.gz"
		if data, err := embeddedModels.ReadFile(embeddedPath); err == nil {
			return loadModelFromBytes(data)
		}
	}

	// Fall back to filesystem: try .gz version first
	gzPath := path + ".gz"
...
// loadModelFromBytes loads a model from compressed JSON bytes
func loadModelFromBytes(data []byte) (*RandomForestModel, error) {
