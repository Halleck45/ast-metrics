...
func (r PhpRunner) DumpAST() {
	...
	fmt.Println(r.filesByDirectory)
	panic("DumpAST")
	...
	for directory, files := range r.filesByDirectory {
		...
		for _, file := range files {
			...
			go func(file string) {
				defer wg.Done()
				r.executePHPCommandForFile(workDir, directory, file)
...
func (r PhpRunner) executePHPCommandForFile(tmpDir string, currentlyAnalysedDirectory string, file string) {
	...
	fmt.Println("executePHPCommandForFile")
	panic("executePHPCommandForFile")
	...
	fmt.Println("file " + file + " has hash " + hash)
